...
class Router 
{
	...
	public function routeQuery() {
		try {
			if (isset($_GET['action'])) {
				if ($_GET['action'] == 'post') {
					...
					// si j'ai un id de billet (récupère l'id en auto-increment qui commence à 1 en bdd), je vais chercher la méthode post($post_id) pour afficher le détail du billet choisi
					if ($post_id > 0) {
				...
				// si insertion logAdmin ok -> j'affiche adminProfile	
					...
					if(isset($_POST['author']) AND !empty($_POST['author']) 
						...
						// vérifie si j'ai un id de commentaire et un id de billet
						$id = (int)$this->getParam($_GET, 'id');
						$post_id = intval($this->getParam($_GET, 'post_id'));
						// si id de commentaire et un id de billet mais des erreurs, je renvoie sur la page adminDashboard 
						if ($id > 0 AND $post_id > 0) {
		...
		// attrape les exceptions "Exception" si existantes avec la variable $e qui représente l'exception lancée	
		} catch (Exception $e) {
